...
class LearningAgent(Agent):
    ...
    def __init__(self, env, learning=False, epsilon=1.0, alpha=0.01):
        ...
        self.num_trial = 1       # Number of trials
    ...
    def get_actions(self, state):
        action_all = [random.choice(self.actions)]
        if state in self.Q:
            _action_all = self.Q[state]
            v = list(_action_all.values())
            k = list(_action_all.keys())
            maxv = max(v)
            action_all = [i for i, act in _action_all.items()if act == maxv]
        return action_all 
    ...
    def choose_action(self, state):
        ...
        if self.learning:
            ...
            if p < self.epsilon:
                ...
                action_all = self.get_actions(state)
                action = random.choice(action_all)
        else:
            action = random.choice(self.actions)
        return action
    ...
    def learn(self, state, action, reward):
        ...
        if self.learning:
            v_new = self.Q[state][action] + self.alpha*(reward -self.Q[state][action])
